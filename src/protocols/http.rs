...
impl HttpProtocol {
    ...
    async fn download_to_temp_with_type(&self, url: &Url, source_type: Option<&str>) -> Result<NamedTempFile> {
        ...
        let content = response
            .bytes()
            .await
            .map_err(|e| TinyEtlError::Connection(format!("Failed to read response body: {}", e)))?;
...
impl Protocol for HttpProtocol {
    ...
    async fn create_source_with_type(&self, url: &Url, source_type: Option<&str>) -> Result<Box<dyn Source>> {
        ...
        std::fs::copy(temp_file.path(), &persistent_path)
            .map_err(|e| TinyEtlError::Io(e))?;
    ...
    fn validate_url(&self, url: &Url) -> Result<()> {
        match url.scheme() {
            "http" | "https" => {},
            _ => return Err(TinyEtlError::Configuration(
                format!("HTTP protocol requires http:// or https:// scheme, got: {}", url.scheme())
            ))
        }
...
mod tests {
    ...
    fn test_extract_extension_from_url() {
        ...
        assert_eq!(protocol.extract_extension_from_url(&url), Some("csv".to_string()));
        
        let url = Url::parse("https://example.com/path/to/file.json").unwrap();
        assert_eq!(protocol.extract_extension_from_url(&url), Some("json".to_string()));
        
        let url = Url::parse("https://example.com/data.parquet").unwrap();
        assert_eq!(protocol.extract_extension_from_url(&url), Some("parquet".to_string()));
